...
void Powerpal::parse_measurement_(const uint8_t *data, uint16_t length) {
    ...
    if (length >= 6) {
        ...
        ESP_LOGI("powerpal_ble", "Timestamp: %d, Pulses: %d, Energy Used: %f kWh",unix_time,pulses_within_interval,total_kwh_within_interval);
...
void Powerpal::gattc_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param) {
  switch (event) {
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT: {
      // auto *pairing_code_char_ = this->parent_->get_characteristic(POWERPAL_SERVICE_UUID, POWERPAL_CHARACTERISTIC_PAIRING_CODE_UUID);
      // if (pairing_code_char_ == nullptr) {
      ...
      // auto *reading_batch_size_char_ = this->parent_->get_characteristic(POWERPAL_SERVICE_UUID, POWERPAL_CHARACTERISTIC_READING_BATCH_SIZE_UUID);
      // if (reading_batch_size_char_ == nullptr) {
      //   ESP_LOGE(TAG, "[%s] No Powerpal service or Reading Batch Size Characteristic found at device, not a POWERPAL..?",
      //             this->parent_->address_str().c_str());
      //   break;
      // } else {
      ...
      // auto *measurement_char_ = this->parent_->get_characteristic(POWERPAL_SERVICE_UUID, POWERPAL_CHARACTERISTIC_MEASUREMENT_UUID);
      // if (measurement_char_ == nullptr) {
      ...
      if (param->read.handle == this->reading_batch_size_char_handle_) {
        ...
        if (param->read.value_len == 4) {
          if (param->read.value[0] != this->reading_batch_size_[0]) {
            ...
            auto status = esp_ble_gattc_write_char(this->parent()->gattc_if, this->parent()->conn_id, this->reading_batch_size_char_handle_,
                                                    sizeof(this->reading_batch_size_), this->reading_batch_size_, ESP_GATT_WRITE_TYPE_RSP,
            ...
            auto status = esp_ble_gattc_register_for_notify(this->parent_->gattc_if, this->parent_->remote_bda, this->measurement_char_handle_);
            if (status) {
              ESP_LOGW(TAG, "[%s] esp_ble_gattc_register_for_notify failed, status=%d", this->parent_->address_str().c_str(), status);
            }
          }
        } else {
    ...
    case ESP_GATTC_WRITE_CHAR_EVT: {
      ...
      if (param->write.handle == this->pairing_code_char_handle_ && !this->authenticated_) {
        ...
        auto read_reading_batch_size_status = esp_ble_gattc_read_char(this->parent()->gattc_if, this->parent()->conn_id, this->reading_batch_size_char_handle_, ESP_GATT_AUTH_REQ_NONE);
        ...
        if (this->battery_ != nullptr) {
          // read battery
          auto read_battery_status = esp_ble_gattc_read_char(this->parent()->gattc_if, this->parent()->conn_id, this->battery_char_handle_, ESP_GATT_AUTH_REQ_NONE);
          ...
          auto notify_battery_status = esp_ble_gattc_register_for_notify(this->parent_->gattc_if, this->parent_->remote_bda, this->battery_char_handle_);
          if (notify_battery_status) {
            ESP_LOGW(TAG, "[%s] esp_ble_gattc_register_for_notify failed, status=%d", this->parent_->address_str().c_str(), notify_battery_status);
          }
        }
        ...
        auto read_led_sensitivity_status = esp_ble_gattc_read_char(this->parent()->gattc_if, this->parent()->conn_id, this->led_sensitivity_char_handle_, ESP_GATT_AUTH_REQ_NONE);
      ...
      ESP_LOGE(TAG, "[%s] Seemed to miss any handle matches, what is the handel?: %d", this->parent_->address_str().c_str(), param->write.handle);
			break;
  ...
  switch (event) {
    ...
    case ESP_GAP_BLE_AUTH_CMPL_EVT: {
      if (param->ble_security.auth_cmpl.success) {
        ...
        auto status = esp_ble_gattc_write_char(this->parent()->gattc_if, this->parent()->conn_id, this->pairing_code_char_handle_,
                                                    sizeof(this->pairing_code_), this->pairing_code_, ESP_GATT_WRITE_TYPE_RSP,
                                                    ESP_GATT_AUTH_REQ_NONE);
        if (status) {
